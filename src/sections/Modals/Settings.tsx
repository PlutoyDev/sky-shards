...
let gsCodeLangs: Record<string, string> = {};

if (parseUrl().gsTrans && import.meta.env.VITE_GS_TRANSLATION_URL) {
  if (import.meta.env.VITE_GS_TRANSLATION_URL)
    fetch((import.meta.env.VITE_GS_TRANSLATION_URL as string) + '?langCodeOnly=true')
      .then(res => res.json())
      .then((response: Record<string, string>) =>
        Object.entries(response).forEach(([code, lang]) => (gsCodeLangs[`${code}-GS`] = lang + ' (GS)')),
      )
      .catch(err => console.error('failed to load language codes', err));
}
...
export default function SettingModal() {
  ...
  return (
    <div className='mb-4 flex flex-col justify-center gap-2 md:grid md:grid-cols-2 [&>div]:w-full [&>div]:px-2'>
      ...
      {/* Language */}
      <div>
        <p className='text-bold w-full md:text-center'>{t('language.title')}</p>
        <select
          className='no-scrollbar select select-primary select-xs mt-1 w-full bg-primary text-primary-content'
        ...
        >
          ...
          ))}
          {
            ...
            Object.keys(gsCodeLangs).map(lang => (
              <option key={lang} value={lang}>
                {gsCodeLangs[lang]}
              </option>
            ))
