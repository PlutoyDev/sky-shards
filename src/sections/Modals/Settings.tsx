...
async function loadLanguage(
  language: string,
  prevLang: string,
  setSettings: (settings: { lang: string }) => void,
...
export default function SettingModal({ hideModal }: ModalProps) {
  ...
  const { lightMode, twelveHourMode, timezone, lang, gsTrans, setSettings } = useSettings();
  ...
  const [languageLoader, setLanguageLoader] = useState<{ loading: boolean; error?: string } | null>(null);

  const setLanguage = useCallback(
    async (language: string) => {
      if (language === lang) return; // No change
      const prevLang = lang;
      if (language === 'en' || (i18next.hasResourceBundle(language, 'shard') && !language.endsWith('-GS'))) {
        ...
        loadLanguage(language, prevLang, setSettings, setLanguageLoader);
      }
    },
    [lang, setSettings],
  );
